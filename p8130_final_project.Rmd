...
```{r}
breastcancer_df |>
  ggplot(aes(x = age, y = survival_months, color = status)) +
  geom_point()
```
...
cv_df = cv_df |>
  ...
  mutate(
    rmse_age = map(test, \(df) survdiff(Surv(survival_months,status) ~ age, data = df)),
    rmse_race = map(test, \(df) survdiff(Surv(survival_months,status) ~ race, data = df)),
    rmse_marital = map(test, \(df) survdiff(Surv(survival_months,status) ~ marital_status, data = df)),
    rmse_t.stage = map(test, \(df) survdiff(Surv(survival_months,status) ~ t_stage, data = df)),
    rmse_n.stage = map(test, \(df) survdiff(Surv(survival_months,status) ~ n_stage, data = df)),
    rmse_sixth.stage = map(test, \(df) survdiff(Surv(survival_months,status) ~ sixth_stage, data = df)),
    rmse_dif = map(test, \(df) survdiff(Surv(survival_months,status) ~ differentiate, data = df)),
    rmse_tumor = map(test, \(df) survdiff(Surv(survival_months,status) ~ tumor_size, data = df)),
    rmse_region_examined = map(test, \(df) survdiff(Surv(survival_months,status) ~ regional_node_examined, data = df)),
    rmse_region_pos = map(test, \(df) survdiff(Surv(survival_months,status) ~ reginol_node_positive, data = df)),
    rmse_a.stage = map(test, \(df) survdiff(Surv(survival_months,status) ~ a_stage, data = df)),
    rmse_estrogen = map(test, \(df) survdiff(Surv(survival_months,status) ~ estrogen_status, data = df)),
    rmse_progesterone = map(test, \(df) survdiff(Surv(survival_months,status) ~ progesterone_status, data = df)),
    rmse_grade = map(test, \(df) survdiff(Surv(survival_months,status) ~ grade, data = df)),

  )

cv_df$rmse_tumor[[1]] |>
  as.data.frame() |>
  ggplot(aes(x = tumor_size, y = "(0-E)^2/V")) +
  geom_point()
