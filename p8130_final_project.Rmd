...
```{r}
cv_df = crossv_mc(breastcancer_df,100) 
...
cv_df = cv_df |>
  ...
  mutate(
    rmse_age = map2_dbl(age_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_race = map2_dbl(race_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_marital = map2_dbl(marital_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_t.stage = map2_dbl(t.stage_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_n.stage = map2_dbl(n.stage_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_sixth.stage = map2_dbl(sixth.stage_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_dif = map2_dbl(dif_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_tumor = map2_dbl(tumor_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_region_examined = map2_dbl(region_examined_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_region_pos = map2_dbl(region_pos_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_a.stage = map2_dbl(a.stage_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_estrogen = map2_dbl(estrogen_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_progesterone = map2_dbl(progesterone_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_grade = map2_dbl(grade_mod, test, \(mod, df) rmse(model = mod, data = df)),
